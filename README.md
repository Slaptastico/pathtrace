...
The current command prints each raw `PATH` entry in lookup order. Empty entries are shown as `<empty>`, and later duplicate entries are marked with the earlier entry they repeat.
